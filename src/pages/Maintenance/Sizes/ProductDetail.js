...
import { useProductsQuery } from '../../../services/maintenance';
...
function ProductDetail(props) {
  ...
  const { data: xhr_data, isSuccess } = useProductsQuery({ id: params.id });
  ...
  useEffect(() => {
    if (isSuccess && !!xhr_data.length) {
      setProductData(xhr_data[0]);
      setImages((xhr_data[0]?.images || '').split('|'));
    }
  }, [isSuccess, xhr_data]);

  document.title = 'Product Details | UN Inventory Management System';
  return (
                          ...
                          {(images || []).map((image, i) => (
                            <SwiperSlide key={i}>
                              <img
                                ...
                                onError={(e) =>
                                  (e.target.src =
                                    'https://themes.themesbrand.com/velzon/react/default/static/media/img-8.70e311a32f5037d68405.png')
                                }
                              />
                            </SwiperSlide>
                          ))}
                      ...
                      <div className="product-nav-slider mt-2">
                        ...
                        >
                          <div className="swiper-wrapper">
                            {(images || []).map((image, i) => {
                              return (
                                <SwiperSlide key={i} className="rounded">
                      ...
                      <div className="d-flex">
                        <div className="flex-grow-1">
                          ...
                          <div className="hstack gap-3 flex-wrap">
                            ...
                            <div className="text-muted">
                              Created at :{' '}
                              <span className="text-body fw-medium">
                                {moment(product_data?.created_at).format('ll')}
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="flex-shrink-0">
                          <div>
                            ...
                            <Link
                              to={`/app/products/${params.id}/edit`}
                              id="TooltipTop"
